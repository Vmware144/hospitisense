      ...
      <div style={{ display: "flex", gap: 16, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <Card style={{ borderLeft: "3px solid var(--danger)" }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: "var(--text-primary)", marginBottom: 12 }}>⚠ Common Issues</div>
              {dept.issues.map((iss, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 8 }}>
                  <span style={{ color: "var(--danger)", fontSize: 12, marginTop: 2 }}>–</span>
                  <span style={{ color: "#fca5a5", fontSize: 13, lineHeight: 1.5 }}>{iss}</span>
                </div>
              ))}
            </Card>
            <Card style={{ borderLeft: "3px solid var(--accent)" }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: "var(--text-primary)", marginBottom: 12 }}>✓ Strengths</div>
              {dept.strengths.map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 8 }}>
                  <span style={{ color: "var(--accent)", fontSize: 12, marginTop: 2 }}>✓</span>
                  <span style={{ color: "#86efac", fontSize: 13, lineHeight: 1.5 }}>{s}</span>
                </div>
              ))}
            </Card>
          </div>
        ...
        <div style={{ width: 270 }}>
          <Card style={{ height: "100%" }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: "var(--text-primary)", marginBottom: 16 }}>🚨 Urgent Issues</div>
            {dept.urgent.length === 0 ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 120, gap: 10, color: "var(--accent)" }}>
                ...
                <div style={{ fontSize: 13 }}>No urgent issues</div>
              </div>
            ) : (
              dept.urgent.map((u, i) => (
                <div key={i} style={{
