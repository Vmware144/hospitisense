    ...
    <div style={{ display: "flex", gap: 20, height: "100%", animation: "fadeSlideIn 0.35s ease" }}>
      ...
      <Card style={{ flex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22, flexWrap: "wrap" }}>
          <span style={{ fontWeight: 700, fontSize: 14, color: "var(--text-primary)" }}>Floor</span>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 16 }}>
            {[["var(--accent)", "No Issues"], ["var(--warn)", "Medium"], ["var(--danger)", "Urgent"]].map(([c, l]) => (
        ...
        {!selected && (
          <div style={{ textAlign: "center", marginTop: 20, fontSize: 12, color: "var(--text-muted)", fontStyle: "italic" }}>
            Click a highlighted room to see issue details
          </div>
        )}
      </Card>
          ...
          {selected ? (
            <>
              <div style={{ fontWeight: 700, fontSize: 16, color: "var(--text-primary)", marginBottom: 20, fontFamily: "var(--font-display)" }}>
                Room {selected.num} Details
              </div>

              {[
                { label: "Issue", value: selected.issue, color: "var(--danger)" },
                { label: "Guest Sentiment", value: `😡 ${selected.sentiment}`, color: "var(--text-primary)" },
              ].map(row => (
            ...
            <div style={{
              ...
              <div style={{ fontSize: 13, color: "var(--text-muted)", lineHeight: 1.7 }}>
                Select a red or yellow room on the floor plan to view issue details and AI-suggested actions
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                {[["var(--danger)", "Urgent"], ["var(--warn)", "Medium"]].map(([c, l]) => (
