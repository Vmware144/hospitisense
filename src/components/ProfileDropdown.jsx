      ...
      {open && (
        <div style={{
          ...
          <div style={{ padding: "8px 0" }}>
            <MenuItem icon="👤" label="Profile & Settings" onClick={() => { onNavigateSettings(); setOpen(false); }} />
            
            <MenuItem icon="🌐" label="Language & Region" onClick={() => { onNavigateSettings(); setOpen(false); }} />
            <div style={{ height: 1, background: "var(--border)", margin: "6px 12px" }} />
            
            <MenuItem icon="🚪" label="Sign Out" onClick={logout} color="var(--danger)" />
          </div>
        </div>
      )}
    </div>
